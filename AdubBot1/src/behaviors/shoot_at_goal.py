...
from behaviors.utsystem import Choice
...
class ShootAtGoal(Choice):
    def __init__(self):
        ...
        self.temp_util_desire_boost = 0

    def util(self, bot) -> float:

        if self.temp_util_desire_boost > 0:
            self.temp_util_desire_boost = max(0, self.temp_util_desire_boost - bot.info.dt)
        elif self.temp_util_desire_boost < 0:
            self.temp_util_desire_boost = min(0, self.temp_util_desire_boost + bot.info.dt)
        ...
        return clip01(own_half_01 + 0.1 * in_position + self.temp_util_desire_boost + kickoff_bias01)
    ...
    def exec(self, bot) -> SimpleControllerState:
        ...
        if not bot.shoot.can_shoot and is_closer_to_goal_than(car.pos, hit_pos, bot.info.team):
            ...
            if enemy_hit_time < 1.5 * my_hit_time:
                self.temp_util_desire_boost -= bot.info.dt
                if bot.do_rendering:
